...
for noise_r in [6]:#range(9, 11):
    for temp in [800]: #[670]:#[400, 550, 670]:#[800, 850]:
        for n in [3]:#[3, 15, 30]:#[5, 10, 30]:#np.arange(2, 32, 1):
            print(n, temp)
            
            r = 1.0/10**(noise_r)
            net = KrotovNet(Kx=10, Ky=10, n_deg=n, m_deg=n, M=20*3, nbMiniBatchs=1, momentum=0*0.6, rate=0.005,
                            temp=temp, rand_init_mean=-0.001, rand_init_std=r, selected_digits=[1, 7, 9])
            ...
            run = 1
            net.train_plot_update(3000, isPlotting=False, isSaving=True, saving_dir=data_dir+"run_"+str(net.selected_digits)+"_n"+str(n)+"_T"+str(temp)+".npz", testFreq=100)
