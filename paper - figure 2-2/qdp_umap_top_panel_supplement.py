...
M_embedding = np.load(data_dir+"/memory_umap_embed_correlation_n"+str(30)+str(selected_digits)+".npy")
...
for t_i in range(0, len(t_s)-1):
    im = ax[t_i//2, t_i%2].scatter(embedding[:, 0], embedding[:, 1], c=keys, cmap="tab10", s=10, marker="*")

    
    for i in range(0, N_mem-1):
        data_pnts = M_embedding[t_s[0]:t_s[t_i+1], i, :]
        ax[t_i//2, t_i%2].plot(data_pnts[:, 0], data_pnts[:, 1], linewidth=1, alpha=0.7, color="k")
        ax[0, 0].set_ylabel("UMAP 2"); ax[1, 0].set_ylabel("UMAP 2"); ax[1, t_i%2].set_xlabel("UMAP 1")

    props = dict(boxstyle='round', facecolor='whitesmoke', alpha=0.5)
    ax[t_i//2, t_i%2].text(0.83, 0.97, r"$t=$"+str(t_s[t_i+1]*10), transform=ax[t_i//2, t_i%2].transAxes, fontsize=14, verticalalignment='top', bbox=props)
    
    ax[t_i//2, t_i%2].plot(M_embedding[t_s[t_i+1], :, 0], M_embedding[t_s[t_i+1], :, 1],
                           marker="o", linestyle="", alpha=1, markeredgecolor="k", markerfacecolor="white", markeredgewidth=1, markersize=4)
