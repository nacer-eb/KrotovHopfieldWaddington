...
for noise_r in [4]: # was 8.
    for temp in temp_range:
        for n in n_range:
            ...
            r = 1.0/10**(noise_r)
            net = KrotovNet(Kx=1, Ky=1, n_deg=n, m_deg=n, M=2, nbMiniBatchs=1, momentum=0*0.6, rate=0.01, temp=temp, rand_init_mean=-0.001, rand_init_std=r, selected_digits=[4, 4])
            ...
            net.visibleDetectors[0] = net.miniBatchs_images[0, 0] #0.5*(net.miniBatchs_images[0, 0] + net.miniBatchs_images[0, 1])
            
            
            net.train_plot_update(1000, isPlotting=False, isSaving=False, saving_dir=data_dir+"l_n"+str(n)+"_T"+str(temp)+".npz", testFreq=100, l_condition=False)
