...
if isFirstRun:
    for a_i, a in enumerate(np.arange(0, 1, 0.1)):
        ...
        print(a_i)        
    np.save(data_dir + "FPs.npy", data[:, :, -1, :, :])
...
im1, = ax.plot(data[:, :, t, 1, 0].ravel(), data[:, :, t, 1, 1].ravel(), linestyle="", marker=".", color="red", ms=20)

def update(t):
    im.set_data(data[:, :, t, 0, 0].ravel(), data[:, :, t, 0, 1].ravel())
    im1.set_data(data[:, :, t, 1, 0].ravel(), data[:, :, t, 1, 1].ravel())
