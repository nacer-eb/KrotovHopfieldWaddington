...
fig = plt.figure(figsize=(27, 65-22+1 ))
...
axs['1'].set_xlabel(r'$\alpha_1$', labelpad=20); axs['3'].set_xlabel(r'$\ell$', labelpad=20);
...
for i, n in enumerate(n_range):
    ...
    for j in range(3):
        f3.plot_snapshot(mem_snapshots_axs[i, j], alphas[j], betas[j], isStable=int(j!=1)+int(i==2), fontsize=24) # if j==1 it's unstable unless i==2 in which case the center point is stable
...
for i in range(0, 2):
    for j in range(0, 4):
        dynamics_nullcline_axes[i, j].set_xlabel(r'$\ell$', labelpad=15);
