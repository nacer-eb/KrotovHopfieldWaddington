...
class Figure_3:
    def __init__(self, saving_dir, data_dir = "data_2_2_2/", C_data_dir = "C_Code_FPs/", selected_digits=[1, 4]):
    ...
    def plot_nullclines(self, ax, n, temp=700, t_0=0, t=0, plotDynamics=False, density=1):
        ...
        ax.contour(self.l_0_mesh, self.alpha_mesh, alpha_nullcline, [0], colors="purple", linewidths=4, alpha=0.5)
        ax.contour(self.l_0_mesh, self.alpha_mesh, l_nullcline, [0], colors="orange", linewidths=4, alpha=0.5)

    
        textbox = ax.text(0.07, 0.94, r"$n=$"+str(n), transform=ax.transAxes, fontsize=18, verticalalignment='top', horizontalalignment='left', bbox=self.props)
