...
from Figure_6_functions import *
...
# Figure 6 object
f5 = Figure_6("run_[1, 4]_n15_T700_alpha0.8_l_00.5.npz", data_dir="../Figure 6/data/", C_data_dir="../Figure 6/C_Code_FPs/") 
...
norm_20 = matplotlib.colors.Normalize(vmin=0, vmax=20)

t=0
for i, n in enumerate(n_range):
    for j, ell in enumerate(ell_range):
        for k, alpha in enumerate(alpha_range):
            u_i = k + j*len(alpha_range)
            points_1[i][j][k], = nullcline_axes[i].plot(data_ells[i, j, k, t, 0], data_coefs[i, j, k, t, 0, 0], linestyle="",
                                                        marker=matplotlib.markers.CARETUPBASE, ms=10, color=cmap_tab20(norm_20(u_i*2)))
            points_2[i][j][k], = nullcline_axes[i].plot(data_ells[i, j, k, t, 1], data_coefs[i, j, k, t, 1, 0], linestyle="",
                                                        marker=matplotlib.markers.CARETDOWNBASE, ms=10, color=cmap_tab20(norm_20(u_i*2+1)))

props = dict(boxstyle='round', facecolor='whitesmoke', alpha=0.5)
fig.suptitle("epoch=0", fontsize=24, bbox=props)
    
for i, n in enumerate(n_range):
    alphas, betas = f5.plot_nullclines(nullcline_axes[i], n, plotDynamics=False)
...
ani = anim.FuncAnimation(fig, update, frames=tmax*0+1000-1, interval=100, blit=False)
ani.save("dynamicsless_nullcline_movie_n"+str(n)+"_NEW.mov", writer="ffmpeg", fps=60)
    
#plt.savefig("Figure_6S_Nullclines.png")
