...
def UMAP_plot(ax):
    ...
    for t_i in range(0, len(t_s)-1):
        ...
        """
        for i in range(0, N_mem-1)[::1]:
            data_pnts = M_embedding[t_s[t_i]:t_s[t_i+1], i, :]
        ...
        ax[t_i].text(0.5, 0.95, r"Epoch: "+str(t_s[t_i]*10), transform=ax[t_i].transAxes, fontsize=34, verticalalignment='top', ha='center', bbox=props)
...
plt.savefig("Figure3_layout_n"+str(n)+".pdf")
