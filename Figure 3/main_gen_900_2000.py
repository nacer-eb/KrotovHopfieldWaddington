...
def single_n(nT_merge):
    ...
    net = KrotovNet(Kx=30, Ky=30, n_deg=n, m_deg=n, M=200*len(selected_digits), nbMiniBatchs=1, momentum=0*0.6, rate=0.005, temp=temp, rand_init_mean=-0.003, rand_init_std=initial_noise, selected_digits=selected_digits)
...
if __name__ == '__main__':
    ...
    if not path.exists(data_dir+prefix):
        ...
        net = KrotovNet(Kx=30, Ky=30, M=200*len(selected_digits), nbMiniBatchs=1, momentum=0*0.6,
    ...
    if not path.isfile(dataset):
        net = KrotovNet(Kx=1, Ky=1, M=200*len(selected_digits), nbMiniBatchs=1, momentum=0*0.6,
                        rate=0.005, rand_init_mean=-0.003, rand_init_std=initial_noise, selected_digits=selected_digits)
