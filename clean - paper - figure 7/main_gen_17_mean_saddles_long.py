...
for noise_r in [5]: # was 8.
    for temp in np.arange(500, 1100, 20): #np.arange(500, 800, 20):#[800]:#np.arange(400, 680, 20):
        for n in np.arange(2, 61, 2):#np.arange(2, 32, 2):
            for k, run in enumerate([1, 7]):
                print(n, temp)
                
                r = 1.0/10**(noise_r)
                net = KrotovNet(Kx=1, Ky=1, n_deg=n, m_deg=n, M=2, nbMiniBatchs=1, momentum=0*0.6, rate=0.001, temp=temp/(100**(1.0/n)), rand_init_mean=-0.001, rand_init_std=r, selected_digits=[1, 7])
                # First run
