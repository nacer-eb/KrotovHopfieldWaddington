...
if not first_run:
    ...
    for digit_index in range(0, 2): # 0 or can be 1
        ...
        ax_mem[digit_index].imshow(merge_data(data_Ms[:, :, digit_picker, :].reshape(15*15, 784), len(temp_range), len(n_range)), cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect)
        ax_sec_mem[digit_index].imshow(merge_data(secondary_Ms[:, :, digit_picker, :].reshape(15*15, 784), len(temp_range), len(n_range)), cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect)
        
        ax_phase_ortho[digit_index].imshow(data_coefs[:, :, digit_picker, digit_picker], cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect) # It is a coincidence (ish) that digit picker works for both indices
