        ...
        for j, n in enumerate(n_range):
            for k in range(3):
                for r in range(N_runs):
                    ...
                    run_prefix = "end_states_ric_g_" + str(r) + "/"
...
for i in range(len(axs_saddles_samples)):
    ...
    if i in [0, 2, 4, 6, 7, 8, 10, 12, 13]:
        print(i)
        axs_saddles_samples[i].set_ylabel(r"$n=$"+str(sample_n[i]), fontsize=31, labelpad=40, verticalalignment='center', ha='center', bbox=props)
...
for t_i, temp in enumerate(temp_range):
    ...
    #axs_saddles_alphas[t_i].set_title("T="+str(temp_range[t_i]))
...
for t_i, temp in enumerate(temp_range):
    ...
    axs_pop_proportion[t_i].set_ylabel("T = "+str(temp), labelpad=1300, bbox=props) #Hacky trick
