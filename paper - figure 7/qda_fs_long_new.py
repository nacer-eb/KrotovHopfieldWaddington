...
if not first_run:
    ...
    for digit_index in range(0, 2): # 0 or can be 1       
        ...
        ax_sec_mem[digit_index].imshow(merge_data(secondary_Ms[::2, ::2, :].reshape(len(n_range[::2])*len(temp_range[::2]), 784), len(n_range[::2]), len(temp_range[::2])), cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect)

        ax_phase_ortho[digit_index].imshow(data_coefs_secondary[:, :, digit_index], cmap="bwr_r", vmin=-1, vmax=1, extent=extent, aspect=aspect) 
