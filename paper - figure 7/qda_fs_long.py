...
if not first_run:
    ...
    for digit_index in range(0, 2): # 0 or can be 1       
        ...
        digit_picker = data_coefs[:, :, :, digit_index] > data_coefs[:, :, :, 1-digit_index]
        ...
        M_AB[digit_index] = data_Ms[digit_picker, :].reshape(len(temp_range), len(n_range), 784)

        data_coefs_secondary = np.copy(data_coefs)
        data_coefs_secondary[:, :, :, digit_index] = 0
        secondary_Ms = data_coefs_secondary@data_T

        ax_mem[digit_index].imshow(merge_data(data_Ms[digit_picker, :], len(n_range), len(temp_range)), cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect)
        ax_sec_mem[digit_index].imshow(merge_data(secondary_Ms[digit_picker, :], len(n_range), len(temp_range)), cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect)

        ax_phase_ortho[digit_index].imshow(data_coefs[digit_picker, (1-digit_index)].reshape(len(temp_range), len(n_range)), cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect) 
