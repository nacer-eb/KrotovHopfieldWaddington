...
if not first_run:
    ...
    for k, run in enumerate([0, 7]):
        data_Ms = np.load(data_dir+"data_Ms_"+str(run)+".npy")
        data_coefs = np.load(data_dir+"data_coefs_"+str(run)+".npy")
        
        
        ax_a1[k].set_title(r"$\alpha_"+str([0, 7][0])+"("+str(run)+")$")
        im = ax_a1[k].imshow(data_coefs[:, :, 0, 1-k], cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect)


    data_coefs_1 = np.load(data_dir+"data_coefs_"+str(0)+".npy"); data_coefs_7 = np.load(data_dir+"data_coefs_"+str(7)+".npy")
    ax_dist.imshow((data_coefs_1[:, :, 0, 0] - data_coefs_7[:, :, 0, 0]), cmap="bwr", vmin=-1, vmax=1, extent=extent, aspect=aspect)
    
    ax_dist.set_title(r"$\alpha_0(0) - \alpha_0(7)$")
    plt.show()
