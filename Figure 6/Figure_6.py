...
#############################################################
#############################################################
data_dir_intra =  "data/[4, 4]_intra/" #"data_[2, 2]_intra/"

digit_classes = [4, 9]#[3, 8]#[4, 7]#[4, 9]
data_dir_inter = "data/[4, 9]_mean_inter/" # "data_"+str(digit_classes)+"_mean_inter/"
...
ax[1].scatter(n, T_calc_rescaled, s=7, color="red")
...
for t_i, t in enumerate([-1, -3, -5]):
    axs['E'].plot(data_ortho[t], marker=".", label=r"$T_r$ = "+'{0:.2f}'.format(temp_range_rescaled[t]), color=colors[t_i])
...
for digit in [0, 1]:
    ...
    T_calc_rescaled = T_calc/784.0
    ax[digit, 1].scatter(n, T_calc_rescaled, s=7, c="red")
    ...
    ax[digit, 0].set_ylabel("Rescaled Temperature", labelpad=20)
    ...
    for t_i, t in enumerate([-1, -5, -8]):
        ax_1d[digit].plot(data_coefs_inter[t, :, digit, 1-digit], marker=".", label=r"$T_r$ = "+'{0:.2f}'.format(temp_range_rescaled[t]), color=colors[t_i])
