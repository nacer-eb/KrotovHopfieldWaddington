...
def single_n(index):
    ...
    net.train_plot_update(1, isPlotting=False, isSaving=True, saving_dir=data_dir+prefix+"trained_net_ic_n"+str(n)+"_T"+str(temp)+"_run"+str(index)+".npz", testFreq=500)

    # Train and Save every epoch- 3500 seems to be good enough and saves space
    net.train_plot_update(3500, isPlotting=False, isSaving=True, saving_dir=data_dir+prefix+"trained_net_n"+str(n)+"_T"+str(temp)+"_run"+str(index)+".npz", testFreq=500)
    
    # This needs a fix 
    generate_umap_embedding(data_dir, prefix, n, temp, index, verbose=True)
    



if __name__ == '__main__':
    indices = np.arange(12, 100, 1, dtype=int)
    print(indices)
