...
def generate_umap_embedding(data_dir, prefix, n, temp, index, isUMAPObjAlreadyGenerated=True, verbose=False):
    ...
    saving_dir = data_dir+prefix+"trained_net_n"+str(n)+"_T"+str(temp)+"_run"+str(index)+".npz"
    ...
    np.save(data_dir+prefix+"memory_umap_embed_correlation_n"+str(n)+"_T"+str(temp)+"_run"+str(index), M_embedding)
   ...
   # else
    M_embedding = np.load(data_dir+prefix+"memory_umap_embed_correlation_n"+str(n)+"_T"+str(temp)+"_run"+str(index)+".npy")
    t = 0 
    ...
    ani.save(data_dir+prefix+"UMAP_movie_n"+str(n)+"_T"+str(temp)+"_run"+str(index)+".mp4", writer="ffmpeg")
