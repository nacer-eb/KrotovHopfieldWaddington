...
class KrotovNet:
    ...
    def get_init_array(self):
        ...
        init_array = np.asarray([self.Kx, self.Ky, self.n_deg, self.m_deg, self.M, self.nbMiniBatchs,
                      self.train_momentum, self.train_rate, self.rand_init_mean,
                      self.rand_init_std])
    ...
    def visibleLayer_train(self, v, t):
        ...
        dV_tmp = 2*self.n_deg*self.m_deg*v_*np.reshape(np.repeat(np.sum(dV_1*dV_2, axis=1), self.N), (self.K, self.N))
        dV = dV_tmp
