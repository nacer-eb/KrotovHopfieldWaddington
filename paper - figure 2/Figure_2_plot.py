...
class Figure_2_panel:
    def __init__(self, n, temp=800, data_dir="../paper - figure 1/data/", selected_digits = [1, 4, 7], t_s = [20, 200, 250, 300, 344] ):
        ...
        self.data_T = np.load(self.data_dir+"miniBatchs_images.npy")[0]


        self.mapper = pickle.load((open(self.data_dir+"/umap_model_correlation.sav", 'rb')))
    ...
    def split_plot(self, ax, t_start, t_stop):
        ...
        ax[0].set_xlabel(r"$\alpha_"+str(self.selected_digits[0])+"$"); ax[1].set_xlabel(r"$\alpha_"+str(self.selected_digits[0])+"$");
