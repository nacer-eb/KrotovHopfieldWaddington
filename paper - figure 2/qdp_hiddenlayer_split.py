...
for i, t in enumerate(t_samples):
    for j in range(0, 100):
        ax[i//2, i%2].plot(data_L[t, j, 1], data_L[t, j, 4], color=tab_10(normalizer(data_L_key[j])), linestyle="", marker=".", ms=3)
        ax[i//2, i%2].plot(data_L[t-trailsize:t, j, 1], data_L[t-trailsize:t, j, 4], color=tab_10(normalizer(data_L_key[j])), linestyle="-", marker="", lw=1)

    ax[i//2, i%2].text(0.83, 0.97, s=r"$t=$"+str(t), fontsize=14, transform=ax[i//2, i%2].transAxes, verticalalignment='top', bbox=props)
    ax[i//2, 0].set_ylabel(r"$L_4$"); ax[-1, i%2].set_xlabel(r"$L_1$")
...
fig.legend(custom_lines, ['Label (Hidden layer)', 'Label trajectory/trail'], loc='upper center', ncol=4)
...
cb.ax.set_ylabel("Digit class post-training.")
plt.show()
...
for j in range(0, 100):
    p[j], = ax.plot(data_L[0, j, 1], data_L[0, j, 4], color=tab_10(normalizer(data_L_key[j])), linestyle="-", marker=".", ms=3)
...
def update(t_):
    t = t_ + 430
    for j in range(0, 100):
        p[j].set_data(data_L[t-20:t, j, 1], data_L[t-20:t, j, 4])

    text.set_text(r"$t=$"+str(t))
        
    return *p,

ani = anim.FuncAnimation(fig, update, frames=300, interval=10, blit=True, repeat=True)
ani.save('hidden_layer_split[1, 4, 7]_2d.mp4', writer='ffmpeg')
#plt.show()
#"""
