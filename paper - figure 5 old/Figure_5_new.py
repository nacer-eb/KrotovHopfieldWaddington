...
selected_digits = [5, 8]#[6, 8]#[2, 3]# [4, 9]#
data_dir = "data_"+str(selected_digits)+"/" #"data_49_mean/"# 

temp_range = np.arange(400+100, 900+100, 20)#temp_range = np.arange(400, 900, 20)
n_range = np.arange(2, 31, 1)#n_range = np.arange(2, 31, 1)
...
fig = plt.figure(figsize=(30, 84-56+1))
...
for digit in [0, 1]:
    ...
    #norm_ortho = matplotlib.colors.Normalize(vmin=np.clip(np.min(data_ortho)-0.01, -1, 0.6), vmax=np.clip(np.max(data_ortho)+0.00, -1, 0.6))
    norm_ortho = matplotlib.colors.Normalize(vmin=np.clip(np.min(data_ortho)-0.01, -1, 0.6), vmax=0.07)
...
plt.savefig("Figure-5_tomerge_"+str(selected_digits)+".png")
