...
prefix = str(selected_digits)+"_constant_IC/" # I used main,and momentum #"main"#
...
def plot_max_alpha(ax):
    ...
    for h, temp in enumerate(temp_range):
        ...
        ax.plot(n_range, filtered, label="T="+str(temp), color=colors[h], lw=5)
        
        ax.scatter(n_range, data_coefs_abs_max, marker=".", color=colors[h], alpha=0.3, edgecolor="k", s=90)
...
def plot_variable_shannon(ax, isFirstRun = True):
    ...
    for h, temp in enumerate(temp_range):
        ...
        ax.plot(n_range, filtered, label="T="+str(temp), color=colors[h], lw=5)
        ax.scatter(n_range, rs_mean[h], marker=".", color=colors[h], alpha=0.73, s=90) #, edgecolor="k"
...
def plot_UMAP(ax, t_i, n_i):
    ax.scatter(embedding[:, 0], embedding[:, 1], c=keys, cmap="tab10", s=120, marker="*") # Plotting the UMAP training data

    data_M = data_Ms[t_i, n_i]
    M_embedding = mapper.transform(data_M)

    ax.plot(M_embedding[:, 0], M_embedding[:, 1], marker="o",
                               linestyle="", alpha=1, markeredgecolor="k", markerfacecolor="white", markeredgewidth=3, markersize=17)
    
    # Time stamps / Cosmetics
    ax.text(0.95, 0.95, r"$n=$"+str(n_range[n_i]), transform=ax.transAxes, fontsize=44, verticalalignment='top', ha='right', bbox=props)
