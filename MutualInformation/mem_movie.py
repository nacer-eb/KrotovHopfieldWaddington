...
im = [0]*3
for h in range(0, 3):
    im[h] = ax[h].imshow(merge_data(data_Ms[h, n_i], 10, 10), cmap='bwr', vmin=-1, vmax=1)
    ax[h].set_title("Temp = " + str(temp_range[h]) + ", n = "+str(n_range[n_i]))
    ax[h].set_xticks([]); ax[h].set_yticks([])
...
def update(n_i):
    ...
    for h in range(0, 3):
        im[h].set_data(merge_data(data_Ms[h, n_i], 10, 10))
        ax[h].set_title("Temp = " + str(temp_range[h]) + ", n = "+str(n_range[n_i]))
    
    return ax[0], ax[1], ax[2], im[0], im[1], im[2]

ani = anim.FuncAnimation(fig, update, frames=len(n_range), interval=100, blit=False, save_count=10) #len(n_range)
ani.save('mem_movie.mp4', writer="ffmpeg")
fig.show()
