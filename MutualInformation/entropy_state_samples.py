...
fig, ax = plt.subplots(3, 4, figsize=(16, 12), dpi=500)

props = dict(boxstyle='round', facecolor='whitesmoke', alpha=0.5)

for t_i in range(0, 3):
    for j, n_i in enumerate([10, 21, 35, 50]):
        print(j)
        print(n_i)
        ax[t_i, j].imshow(merge_data(data_Ms[t_i, n_i, index], 6, 1), cmap="bwr", vmin=-1, vmax=1)
